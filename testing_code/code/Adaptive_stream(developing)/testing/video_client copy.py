...
client_socket3.sendto(str(fps).encode('utf-8'),(host_ip,port3))
client_socket2.sendto(para,(host_ip,port2))
feedback,_ = client_socket3.recvfrom(BUFF_SIZE3)
print(feedback)
...
while True:
	...
	if cnt == frames_to_count:
		try:
			...
			pass
	cnt+=1
	#c																	lient_socket2.sendto(quality.encode('utf-8'),(host_ip,port2))
	client_socket2.sendto(para,(host_ip,port2))
	client_socket3.sendto(str(fps).encode('utf-8'),(host_ip,port3))
	print ("RECEIVED FPS--->:		", fps)

	#change,_ = client_socket2.recvfrom(BUFF_SIZE3)
	#print("Feedback from server to change parameters:", change)
